...
namespace asx {
    namespace eeprom {
        ...
        static inline void erase() {
            ccp_write_io((uint8_t *)&NVMCTRL.CTRLA, NVMCTRL_CMD_EEERASE_gc);
        }   
        ...
        */
        ...
        class Storage : public T, public Operation {
            ...
            static uint16_t calc_fletcher16() {
        ...
        public:
            ...
            explicit Storage(const T& initial_data) : T{initial_data} {
                ...
                uint16_t fl16 = calc_fletcher16();

                // Update the content
                if ( *fletcher16_ptr != fl16 ) {
                    memcpy(data, (void *)static_cast<T*>(this), size);
                    *fletcher16_ptr = fl16;
                    NVMCTRL.ADDR = (uint16_t)(&USERROW);
                    wait_til_ready();
                    ccp_write_io((uint8_t *)&NVMCTRL.CTRLA, NVMCTRL_CMD_PAGEERASEWRITE_gc);
                } else {
                    memcpy((void *)static_cast<T*>(this), data, size);
                }
            }
        ...
        protected:
            ...
            virtual void do_operation() override {
                // Check the eeprom content
                memcpy(data, (void *)static_cast<T*>(this), size);
                *fletcher16_ptr = calc_fletcher16();
                NVMCTRL.ADDR = (uint16_t)(&USERROW);
                ccp_write_io((uint8_t *)&NVMCTRL.CTRLA, NVMCTRL_CMD_PAGEERASEWRITE_gc);
            }
        };
